...
async function main() {
  ...
  try {
    ...
    const login = await call(port, 'POST', '/api/auth/login', { username: 'sandboxuser', password: 'AvangridUser1!' });
    ...
    ok('cloud_provider defaulted to Microsoft Azure', wsRes.body && wsRes.body.cloud_provider === 'Microsoft Azure');
    ...
    const ucPayload = {
      ...
      sources: 'ERP', dataavail: 'Readily available & clean', integrations: ['Microsoft 365', 'Microsoft Fabric'], realtime: 'No', technotes: 'n/a',
    ...
    const adv = await call(port, 'PUT', `/api/use-cases/${ucId}/advisory`, {
      tier: 'Tier 2', verdict_name: 'Advance', recommended_platform: 'Azure AI Foundry',
      gate_resolved: 'feasibility', reasoning: { why: 'clean data' }, journey: { steps: ['pilot'] },
    });
    ok('advisory upsert ok', adv.status === 200 && adv.body && adv.body.recommended_platform === 'Azure AI Foundry');
