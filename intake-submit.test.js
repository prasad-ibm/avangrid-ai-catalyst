 ...
 * wait for AAIC_API.createUseCase to resolve, capture the REAL uc.id, persist it
 * as aaic_use_case_id, and navigate to bxt.html?id=<newId> — never bare bxt.html
  ...
  return new Promise(resolve => {
    ...
    const dom = new JSDOM(html, {
      ...
      beforeParse(w){
        ...
        w.localStorage.setItem('aaic_use_case_id', 'ASKHR_STALE');
        w.AAIC_API = {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        ok('aaic_use_case_id persisted as NEW123', w.localStorage.getItem('aaic_use_case_id') === 'NEW123');
        resolve();
      }, 30);
    }, 60);
  });
...
function testSlow(){
  return new Promise(resolve => {
    ...
    const dom = new JSDOM(html, {
      ...
      beforeParse(w){
        w.localStorage.setItem('aaic_use_case_id', 'ASKHR_STALE');
        w.AAIC_API = { createUseCase: () => new Promise(() => {}) }; // never resolves
      }
    });
    setTimeout(() => {
      const w = dom.window;
      ...
      setTimeout(() => {
        ...
        ok('stale AskHR id was cleared at submit start (not reused)',
           w.localStorage.getItem('aaic_use_case_id') === null);
        resolve();
      }, 50);
    }, 60);
  });
...
function testOffline(){
  return new Promise(resolve => {
    ...
    const dom = new JSDOM(html, {
      ...
      beforeParse(w){
        w.localStorage.setItem('aaic_use_case_id', 'ASKHR_STALE');
        w.AAIC_API = { createUseCase: () => Promise.resolve({ _offline:true, data:{} }) };
      }
    });
    setTimeout(() => {
      const w = dom.window;
...
function testBxtHandoff(){
  return new Promise(resolve => {
    ...
    var mock = 'window.AAIC_API={getUseCase:function(id){window.__lastFetchId=id;return Promise.resolve('
    ...
    var dom = new JSDOM(htmlB, {
      ...
      beforeParse(w){ w.localStorage.setItem('aaic_use_case_id', 'ASKHR_STALE'); }
    });
    setTimeout(() => {
...
// ---------------------------------------------------------------------------
// Test 5 (H3 workspace_id) — no cached aaic_workspace_id. Submit must call
// listWorkspaces(), pick the Avangrid workspace, cache its id, and carry THAT
...
function testWorkspaceResolution(){
  return new Promise(resolve => {
    console.log('\n== 5. no aaic_workspace_id -> resolves Avangrid workspace for the POST ==');
    const WORKSPACES = [
      { id:'WS_ACME', name:'Acme Corp' },
      { id:'WS_INTEL', name:'Avangrid' },
    ...
    const dom = new JSDOM(html, {
      ...
      url:'https://example.com/intake.html', virtualConsole: quietConsole(),
      beforeParse(w){
        // Fresh user: NO aaic_workspace_id set (never ran setup).
        w.AAIC_API = {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ok('POST body carried the Avangrid workspace_id', postedBody && postedBody.workspace_id === 'WS_INTEL');
        ok('aaic_workspace_id cached as the Avangrid id', w.localStorage.getItem('aaic_workspace_id') === 'WS_INTEL');
...
// ---------------------------------------------------------------------------
// Test 6 (H3 workspace_id) — a cached aaic_workspace_id must be used verbatim
// WITHOUT calling listWorkspaces (no redundant network round-trip).
// ---------------------------------------------------------------------------
function testWorkspaceCached(){
  return new Promise(resolve => {
    console.log('\n== 6. cached aaic_workspace_id is used without listing ==');
    ...
    const dom = new JSDOM(html, {
      ...
      url:'https://example.com/intake.html', virtualConsole: quietConsole(),
      beforeParse(w){
        w.localStorage.setItem('aaic_workspace_id', 'WS_CACHED');
        w.AAIC_API = {
