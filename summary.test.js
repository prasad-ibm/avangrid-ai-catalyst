...
function newDom(intake, bxt, feas, advisory) {
  return new JSDOM(html, {
    ...
    beforeParse(w) {
      ...
      if (bxt      !== undefined) w.localStorage.setItem('aaic_bxt',         JSON.stringify(bxt));
      if (feas     !== undefined) w.localStorage.setItem('aaic_feasibility', JSON.stringify(feas));
      if (advisory !== undefined) w.localStorage.setItem('aaic_advisory',    JSON.stringify(advisory));
    }
  });
}
...
const ADV  = { tier:'Extend', verdictName:'Scale Smart', platform:'Power Apps / Copilot Studio', compliance:{ label:'COMPLIANT', ok:true } };
...
ok('storage keys use aaic_ prefix', api.SUMMARY_KEY === 'aaic_summary');
...
ok('AWAF averages BXT B/X/T (80,72,78 -> ~77)', fws[0].score === Math.round((80+72+78)/3));
...
console.log('\n== 10. Persistence: writes aaic_summary for Gate 6 ==');
const stored = JSON.parse(dom.window.localStorage.getItem('aaic_summary'));
ok('aaic_summary persisted', !!stored);
...
ok('re-run keeps aaic_summary valid', !!JSON.parse(dom.window.localStorage.getItem('aaic_summary')).roi);
...
ok('demo writes aaic_summary', !!domD.window.localStorage.getItem('aaic_summary'));

console.log('\n== 15. Zero foreign-vendor strings; Microsoft framing present ==');
ok('contains Avangrid framing (AWAF / Power Apps / Copilot Studio / Cloud Adoption Framework)',
   /(AWAF|Power Apps \/ Copilot Studio|Microsoft Cloud Adoption Framework|Responsible-AI)/.test(html));
