...
const HIGH = {
  ...
  dataavail: 'Readily available & clean', integrations: ['Microsoft 365','Microsoft Fabric','Azure AI Foundry'],
...
function newDom(seedIntake){
  return new JSDOM(html, {
    ...
    beforeParse(w){ if (seedIntake !== undefined) w.localStorage.setItem('aaic_intake', JSON.stringify(seedIntake)); }
  });
}
...
setTimeout(() => {
  ...
  const NESTED_POP = {
    ...
    technical_context: { sources: ['Structured DB'], dataavail: 'Readily available & clean', integrations: ['Microsoft 365','Microsoft Fabric','Azure AI Foundry'], realtime: false, technotes: '' },
  ...
  ok('normalizeIntake lifts technical_context.integrations → integrations', JSON.stringify(norm.integrations) === JSON.stringify(['Microsoft 365','Microsoft Fabric','Azure AI Foundry']));
  ...
  // Full load path: a NESTED payload persisted to localStorage['aaic_intake']
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      console.log('\n== 10. Reads aaic_intake key + no foreign-vendor strings ==');
      ok('bxt.html references aaic_intake key', /aaic_intake/.test(html));
      ok('contains Avangrid framing (Azure AI Foundry / Copilot / Copilot Studio / Power Apps)',
         /(Azure AI Foundry|Copilot|Copilot Studio|Power Apps)/.test(html));
      ...
      console.log('\n== 11. intake.html submit navigates to bxt.html + writes aaic_intake ==');
      ...
      ok('intake persists to aaic_intake on submit', /localStorage\.setItem\(LS_KEY/.test(intakeHtml) && /LS_KEY\s*=\s*['"]aaic_intake['"]/.test(intakeHtml));
      ...
      setTimeout(() => {
        ...
        const domNew = new JSDOM(html, { runScripts:'dangerously', pretendToBeVisual:true, url:'https://example.com/bxt.html',
          beforeParse(w){
            w.localStorage.setItem('aaic_intake', JSON.stringify(HIGH));
            w.localStorage.setItem('aaic_bxt', JSON.stringify({verdict:'PASS'}));
            w.localStorage.setItem('aaic_feasibility', '1');
            w.localStorage.setItem('aaic_advisory', '1');
            w.localStorage.setItem('aaic_summary', '1');
          }});
        setTimeout(() => {
          ...
          ok('new case clears aaic_intake', wN.localStorage.getItem('aaic_intake') === null);
          ok('new case clears aaic_bxt', wN.localStorage.getItem('aaic_bxt') === null);
          ok('new case clears aaic_feasibility', wN.localStorage.getItem('aaic_feasibility') === null);
          ok('new case clears aaic_advisory', wN.localStorage.getItem('aaic_advisory') === null);
          ok('new case clears aaic_summary', wN.localStorage.getItem('aaic_summary') === null);
