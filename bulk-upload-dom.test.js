...
async function main() {
  ...
  window.fetch = async (url, opts) => {
    if (String(url).indexOf('/api/workspaces') !== -1) {
      return { ok: true, status: 200, json: async () => ([{ id: 'ws-1', name: 'Avangrid' }, { id: 'ws-2', name: 'Acme' }]) };
    }
    return realFetch(url, opts);
  };
  global.fetch = window.fetch;

  // Load the module (its browser branch attaches window.AAIC_BULK).
  const code = fs.readFileSync(path.join(__dirname, 'assets', 'bulk-upload.js'), 'utf8');
  window.eval(code);

  ok('window.AAIC_BULK defined', !!window.AAIC_BULK && typeof window.AAIC_BULK.mount === 'function');

  window.AAIC_BULK.mount({ open: '#btn', workspaceId: 'ws-2', onInserted: () => { window.__inserted = true; } });
  ...
  ok('countMissingName helper returns 1', window.AAIC_BULK && window.AAIC_BULK.countMissingName([{name:'x'},{name:''},{name:'  '}]) === 2);
