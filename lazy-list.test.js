/* Tests for assets/lazy-list.js (window.AAIC_LAZY) — chunked rendering. */
"use strict";
...
window.eval(src);
const AAIC_LAZY = window.AAIC_LAZY;
const doc = window.document;

let pass = 0, fail = 0;
function ok(m, c) { if (c) { pass++; console.log('  \u2713 ' + m); } else { fail++; console.log('  \u2717 ' + m); } }

console.log('\n=== AAIC_LAZY chunked rendering ===\n');
ok('module present', !!AAIC_LAZY && typeof AAIC_LAZY.create === 'function');
...
let threw = false;
try { const n = AAIC_LAZY.create({ items: rows, renderItem: function () { return 'x'; } }); n.showMore(); n.setItems([1,2]); }
catch (e) { threw = true; }
