...
const CANON = {
  ...
  advisory_tier: 'Adopt', recommended_platform: 'Azure AI Foundry',
  ...
  return new JSDOM(html, {
    ...
    beforeParse(w) {
      w.fetch = function (url) {
        ...
        if (u.indexOf('/api/workspaces') !== -1) body = [{ id: 'ws-avangrid', name: 'Avangrid Corp' }];
...
const kan = kanDom.window.AAIC_KANBAN;
const kanDoc = kanDom.window.document;
ok('window.AAIC_KANBAN exposed', !!kan);
