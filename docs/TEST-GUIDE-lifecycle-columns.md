...
> The template now has **33 columns** (was 31) and ships **6 example rows** — the 5 Avangrid examples (active) **plus one DELIVERED example** (`Automated Invoice Matching (DELIVERED EXAMPLE)`, `status=completed`, `delivered_at=2026-03-15`) so the format is self-documenting.
