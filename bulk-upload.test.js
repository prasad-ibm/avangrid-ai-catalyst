 ...
 * unique name prefix — so only the 5 seeded Avangrid use cases remain.
 */
...
after(async () => {
  ...
  try {
    ...
    // belt-and-braces guard so the DB is left with only the 5 Avangrid use cases.
    if (tempWorkspaceId) {
  ...
  const rows = [
    {
      ...
      contact_email: 'askhr@avangrid.test',
