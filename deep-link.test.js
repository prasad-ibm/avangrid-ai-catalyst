 ...
 * Loads the module inside jsdom, mocks AAIC_API, and exercises:
...
const API_ROW = {
  ...
  advisory: {
    tier: 'Extend', verdict_name: 'Scale Smart', recommended_platform: 'Power Apps / Copilot Studio',
  ...
  summary: {
    ...
    frameworks: [{ key: 'gadf', name: 'AWAF', score: 77 }, { key: 'caf', name: 'CAF', score: 71 }],
...
function newDom(url, api) {
  ...
  if (api !== undefined) dom.window.AAIC_API = api;
...
let DL = dom.window.AAIC_DEEPLINK;
ok('window.AAIC_DEEPLINK exists', !!DL);
...
ok('trims whitespace', newDom('https://x/summary.html?id=%20uc-9%20').window.AAIC_DEEPLINK.getId() === 'uc-9');
ok('no id -> null', newDom('https://x/summary.html').window.AAIC_DEEPLINK.getId() === null);
ok('empty id -> null', newDom('https://x/summary.html?id=').window.AAIC_DEEPLINK.getId() === null);
...
ok('recommended_platform -> platform', adv.platform === 'Power Apps / Copilot Studio');
...
const loadOpts = await newDom('https://x/summary.html?id=uc-42', mockApi).window.AAIC_DEEPLINK.load();
...
const noId = await newDom('https://x/summary.html', mockApi).window.AAIC_DEEPLINK.load();
ok('no ?id -> resolves null', noId === null);
const noApi = await newDom('https://x/summary.html?id=uc-42', undefined).window.AAIC_DEEPLINK.load();
ok('no AAIC_API -> resolves null', noApi === null);
const errApi = { getUseCase: function () { return Promise.reject(new Error('boom')); } };
const errRes = await newDom('https://x/summary.html?id=uc-42', errApi).window.AAIC_DEEPLINK.load();
...
d.window.localStorage.setItem('aaic_use_case_id', 'stale-1');
ok('URL ?id beats localStorage', d.window.AAIC_DEEPLINK.getUcId() === 'uc-99');
ok('URL id is persisted to localStorage', d.window.localStorage.getItem('aaic_use_case_id') === 'uc-99');
// No URL id -> fall back to localStorage.
d = newDom('https://x/advisory.html');
d.window.localStorage.setItem('aaic_use_case_id', 'ls-7');
ok('no ?id -> localStorage fallback', d.window.AAIC_DEEPLINK.getUcId() === 'ls-7');
// Neither -> null.
ok('no ?id + empty localStorage -> null', newDom('https://x/advisory.html').window.AAIC_DEEPLINK.getUcId() === null);
...
d.window.AAIC_DEEPLINK.wireNav('uc-77');
...
ok('id is URL-encoded', (function(){
  ...
  dd.window.AAIC_DEEPLINK.wireNav('a b/c');
...
ok('falsy id -> no-op (links unchanged)', (function(){
  var dd = newDom('https://x/feasibility.html');
  dd.window.document.body.innerHTML = '<a id="c" href="advisory.html">c</a>';
  dd.window.AAIC_DEEPLINK.wireNav(null);
...
g1.window.AAIC_DEEPLINK.wireNav(g1.window.AAIC_DEEPLINK.getUcId());
...
ok('advisory resolves the SAME id (no drift to demo)', g2.window.AAIC_DEEPLINK.getUcId() === 'CASE-X');
