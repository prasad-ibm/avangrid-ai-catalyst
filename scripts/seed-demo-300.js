 ...
 *
 * Creates a dedicated workspace ("Avangrid - Portfolio Demo") and N fully-evaluated
 ...
 * It does NOT touch any other workspace (e.g. the real "Avangrid" 5-case seed).
 *
...
const WS_NAME = process.env.WS_NAME || 'Avangrid - Portfolio Demo';
...
function makeUseCase(i) {
  ...
  return {
    ...
    contact_email: `${department.toLowerCase().replace(/[^a-z]+/g, '.')}@avangrid.com`,
    ...
    technical_context: { platform: pick(['Microsoft 365 Copilot', 'Copilot Studio', 'Azure AI Foundry Agent Service', 'Power Apps']), complexity: pick(['Low', 'Medium', 'High']) },
    ...
    advisory: { tier: pick(TIERS), verdict_name: pick(['Start Simple', 'Build Custom', 'Scale Native']), recommended_platform: pick(['Microsoft 365 Copilot', 'Copilot Studio', 'Azure AI Foundry Agent Service']), gate_resolved: 'gate1_adopt',
      ...
      journey: [{ phase: 'Adopt', platform: 'Microsoft 365 Copilot', mandate: 'Deploy' }, { phase: 'Extend', platform: 'Copilot Studio', mandate: 'Add actions' }, { phase: 'Scale', platform: 'Azure AI Foundry', mandate: 'If needed' }] },
...
// ---------------------------------------------------------------------------
//  INSERT PATH  (mirrors scripts/seed-avangrid.js, plus status/delivered_at)
// ---------------------------------------------------------------------------
async function recreateWorkspace() {
  ...
  const vals = [WS_NAME, 'Semiconductors / Manufacturing', '100,000+', '$50B+', 'Microsoft Azure', 'Enterprise Plus',
