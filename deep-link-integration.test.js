 ...
 * with ?id=<use_case_id>, fetch that case via AAIC_API and render its REAL
 * persisted data (not the demo/localStorage default).
 *
 * jsdom does not fetch external <script src>, so we inline the real
 * assets/deep-link.js and a MOCK AAIC_API ahead of each page's inline IIFE by
...
const API_ROW = {
  ...
  summary: {
    ...
    frameworks: [{ key: 'gadf', name: 'AWAF', score: 76 }, { key: 'caf', name: 'CAF', score: 70 },
...
function inlineScripts(html) {
  const mock =
    'window.AAIC_API = { getUseCase: function(id){ window.__lastFetchId = id; ' +
...
(async function () {
  ...
  ok('aaic_use_case_id stored for downstream saves',
     sDom.window.localStorage.getItem('aaic_use_case_id') === 'uc-deep-77');
  ok('AWAF framework score derives from fetched BXT (82,70,76 -> 76)',
  ...
  ok('aaic_use_case_id stored', pDom.window.localStorage.getItem('aaic_use_case_id') === 'uc-deep-77');
