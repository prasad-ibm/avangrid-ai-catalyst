...
function newDom(summary, intake) {
  return new JSDOM(html, {
    ...
    beforeParse(w) {
      if (summary !== undefined) w.localStorage.setItem('aaic_summary', JSON.stringify(summary));
      if (intake  !== undefined) w.localStorage.setItem('aaic_intake',  JSON.stringify(intake));
    }
  });
}
...
const SUMMARY = {
  ...
  frameworks: [
    { key:'gadf',      name:'AWAF v2 (Azure Well-Architected Framework)', score:77, band:'Strong' },
    { key:'caf',       name:'Microsoft Cloud Adoption Framework',        score:72, band:'Ready' },
...
ok('storage keys use aaic_ prefix', api.SUMMARY_KEY === 'aaic_summary' && api.INTAKE_KEY === 'aaic_intake');
...
ok('brief pulls P50 ROI (+141%) with $ value', /\+141%/.test(d.getElementById('briefP50').textContent) && /\$/.test(d.getElementById('briefP50').textContent));
ok('brief pulls P10 & P90 band with $ values', /\+62%/.test(d.getElementById('briefP10').textContent) && /\+233%/.test(d.getElementById('briefP90').textContent) && /\$/.test(d.getElementById('briefP10').textContent) && /\$/.test(d.getElementById('briefP90').textContent));
ok('brief verdict line matches derivation', /CONDITIONAL GO/.test(d.getElementById('briefVerdictLine').textContent));
ok('brief conditions name Azure Confidential Computing', /Azure Confidential Computing/.test(secs[2].textContent));
...
ok('wordmark "Avangrid AI Catalyst"', /Avangrid <b>AI Catalyst<\/b>/.test(html));
...
apiD.buildBrief();
ok('demo brief P50 present (+141%) with $ value', /\+141%/.test(dD.getElementById('briefP50').textContent) && /\$/.test(dD.getElementById('briefP50').textContent));
ok('demo brief names invoice reconciliation', /Automated invoice reconciliation/.test(dD.getElementById('brief').textContent));

console.log('\n== 17. Zero foreign-vendor strings; Microsoft framing present ==');
ok('contains Avangrid framing (Azure Confidential Computing / Azure AI Foundry / Copilot / Microsoft Azure)',
   /(Azure Confidential Computing|Azure AI Foundry|Copilot|Microsoft Azure)/.test(html));
const microsoft = ['Google','Gemini','Vertex','AppSheet','Agentspace','BigQuery','GADF','watsonx','IBM','AWS','Bedrock'];
