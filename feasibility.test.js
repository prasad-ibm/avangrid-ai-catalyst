...
setTimeout(() => {
  ...
  ok('all-5 path = Power Apps / Copilot Studio', /Power Apps \/ Copilot Studio/.test(cdHigh.path));
  ...
  ok('low path = Azure AI Foundry (pro engineering)', /Azure AI Foundry \(pro engineering\)/.test(cdLow.path));
  ...
  console.log('\n== 13. Data flow: reads aaic_intake + aaic_bxt, seeds, writes aaic_feasibility ==');
  ...
  // wrote aaic_feasibility on init
  const persisted = JSON.parse(window.localStorage.getItem('aaic_feasibility'));
  ok('aaic_feasibility persisted on load', persisted && typeof persisted.composite === 'number');
  ...
  setTimeout(() => {
    ...
    ok('demo writes aaic_feasibility', !!domD.window.localStorage.getItem('aaic_feasibility'));

    console.log('\n== 16. Zero foreign-vendor strings; Microsoft framing present ==');
    ok('contains Avangrid framing (Azure AI Foundry / Copilot / Copilot Studio / Power Apps / Microsoft Fabric)',
       /(Azure AI Foundry|Copilot|Copilot Studio|Power Apps|Microsoft Fabric|Microsoft 365)/.test(html));
