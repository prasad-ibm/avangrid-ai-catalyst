...
function mapWorkspace(body) {
  ...
  return {
    ...
    cloud_provider: state.cloud ?? state.cloud_provider ?? 'Microsoft Azure',
...
/* -------------------------------------------------------------------------- */
/* AI (Copilot) — status is public; generation routes require auth (below the   */
...
// use_cases / intake fields and the file ships with the 5 existing Avangrid use
...
async function bootstrapAuth() {
  ...
  const seedPass = process.env.SEED_PASSWORD || 'AvangridUser1!';
...
function start(port) {
  ...
  return app.listen(p, () => {
    console.log(`Avangrid AI Catalyst backend listening on port ${p}`);
  });
}
