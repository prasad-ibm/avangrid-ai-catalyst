/*
 * AAIC deep-link helper — additive, optional. Exposes window.AAIC_DEEPLINK.
 *
 ...
 * API (via AAIC_API), and maps the flat DB rows BACK into the in-memory shapes
...
(function () {
  ...
  function mapPanelSummary(s, intake) {
    ...
    if (Array.isArray(s.frameworks)) {
      ...
      var NAMES = { gadf: 'AWAF v2', google_caf: 'Microsoft Azure Adoption', mckinsey_mit: 'McKinsey \u00d7 MIT Sloan', gartner: 'Gartner' };
  ...
  function mapUseCase(row) {
    ...
    // The offline fallback in AAIC_API.getUseCase returns a different (localStorage)
    ...
    if (row._offline) {
      // localStorage aaic_summary is already in panel shape, so pass it through.
      return {
  ...
  function load() {
    ...
    if (!window.AAIC_API || typeof window.AAIC_API.getUseCase !== 'function') {
      return Promise.resolve(null);
    }
    return window.AAIC_API.getUseCase(id)
      .then(function (row) {
