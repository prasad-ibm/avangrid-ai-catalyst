/*
 * AAIC use-case picker — additive, optional. Exposes window.AAIC_UCPICKER.
 *
 ...
 *
 * Usage: window.AAIC_UCPICKER.mount('#ucPicker')
 *   - Populates the given <select> from window.AAIC_API.listPortfolio(wsId)
...
(function () {
  ...
  function currentId() {
    try {
      if (window.AAIC_DEEPLINK && typeof window.AAIC_DEEPLINK.getId === 'function') {
        return window.AAIC_DEEPLINK.getId();
      }
  ...
  function currentWorkspaceId() {
    try {
      ...
      if (window.AAIC_WORKSPACE_ID) return window.AAIC_WORKSPACE_ID;
    } catch (e) { /* ignore */ }
    return null;
  }

  // Fetch the portfolio array. Uses AAIC_API when available; falls back to a
  // raw fetch. Always resolves to an array (empty on any failure).
  function loadPortfolio(wsId) {
    if (window.AAIC_API && typeof window.AAIC_API.listPortfolio === 'function') {
      return window.AAIC_API.listPortfolio(wsId).then(function (rows) {
  ...
  window.AAIC_UCPICKER = { mount: mount };
