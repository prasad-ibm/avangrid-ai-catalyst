...
setTimeout(() => {
  ...
  // pick microsoft-native integrations
  document.querySelectorAll('.chips[data-key="integrations"] .chip').forEach(c=>{ if(['Microsoft 365','Microsoft Fabric'].includes(c.dataset.val)) click(c); });
  const platTextHigh = document.getElementById('cdPlatform').textContent;
  ok('platform line references a Avangrid platform', /(Power Apps|Copilot Studio|Azure AI Foundry)/.test(platTextHigh));

  console.log('\n== 4. localStorage save/restore ==');
  const raw = window.localStorage.getItem('aaic_intake');
  ok('aaic_intake key present', !!raw);
  const saved = JSON.parse(raw);
  ok('saved.value === ">$5M"', saved.value === '>$5M');
  ok('saved.align stored', saved.align === 'Core strategic priority');
  ok('saved.integrations is array with Microsoft Fabric', Array.isArray(saved.integrations) && saved.integrations.includes('Microsoft Fabric'));
  ...
  const storage = window.localStorage.getItem('aaic_intake');
  const dom2 = new JSDOM(html, { runScripts:'dangerously', pretendToBeVisual:true, url:'https://example.com/intake.html',
    beforeParse(w){ w.localStorage.setItem('aaic_intake', storage); } });
  setTimeout(() => {
    ...
    ok('restore: Microsoft Fabric chip active',
       Array.from(d2.querySelectorAll('.chips[data-key="integrations"] .chip')).some(c=>c.dataset.val==='Microsoft Fabric' && c.classList.contains('is-on')));

    console.log('\n== 5. Microsoft remap strings present, no foreign-vendor strings ==');
    const H = html; // check raw source for static strings
    ok('contains "Power Apps"', /Power Apps/.test(H));
    ok('contains "Copilot Studio"', /Copilot Studio/.test(H));
    ok('contains "Azure AI Foundry"', /Azure AI Foundry/.test(H));
    ok('contains "Microsoft Cloud Adoption Framework"', /Microsoft Cloud Adoption Framework/.test(H));
    ok('contains "AWAF"', /AWAF/.test(H));
    ok('contains "Microsoft Fabric" integration', /Microsoft Fabric/.test(H));
    const microsoft = ['Google','Gemini','Vertex','AppSheet','Agentspace','BigQuery','GADF','watsonx','Agentforce','Salesforce'];
    microsoft.forEach(m => ok('NO foreign-vendor string "'+m+'"', !new RegExp(m.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')).test(H)));
    ...
    setTimeout(() => {
      ...
      const domG = new JSDOM(html, { runScripts:'dangerously', pretendToBeVisual:true, url:'https://example.com/intake.html',
        beforeParse(w){
          ...
          w.localStorage.setItem('aaic_feasibility', JSON.stringify({score:88}));
          w.localStorage.setItem('aaic_advisory', JSON.stringify({platform:'Azure AI Foundry'}));
          w.localStorage.setItem('aaic_summary', JSON.stringify({done:true}));
        }});
      setTimeout(() => {
        const wG = domG.window;
        ok('aaic_bxt cleared on intake load', wG.localStorage.getItem('aaic_bxt') === null);
        ok('aaic_feasibility cleared on intake load', wG.localStorage.getItem('aaic_feasibility') === null);
        ok('aaic_advisory cleared on intake load', wG.localStorage.getItem('aaic_advisory') === null);
        ok('aaic_summary cleared on intake load', wG.localStorage.getItem('aaic_summary') === null);
