...
const INTAKE_STATE = {
  ...
  sources:['Structured DB','Documents/PDFs','Chat/Tickets','Audio/Video'], integrations:['Microsoft 365','Microsoft Fabric'],
...
function newDom(seedIntake, seedBxt, seedFeas){
  return new JSDOM(html, {
    ...
    beforeParse(w){
      if (seedIntake !== undefined) w.localStorage.setItem('aaic_intake', JSON.stringify(seedIntake));
      if (seedBxt !== undefined) w.localStorage.setItem('aaic_bxt', JSON.stringify(seedBxt));
      if (seedFeas !== undefined) w.localStorage.setItem('aaic_feasibility', JSON.stringify(seedFeas));
    }
  });
}
...
setTimeout(() => {
  ...
  ok('Avangrid wordmark present', /Avangrid/.test(document.querySelector('.gc-header__wordmark').textContent));
  ...
  ok('theme persisted to aaic_theme', window.localStorage.getItem('aaic_theme') === 'light');
  ...
  console.log('\n== 3. Gate banner references AWAF (and NOT MAIDF) ==');
  ok('banner title "Platform Advisory"', /Platform Advisory/.test(document.querySelector('.banner__title').textContent));
  ok('banner names Azure Well-Architected Framework (AWAF)', /Azure Well-Architected Framework \(AWAF\)/.test(document.querySelector('.banner__desc').innerHTML));
  ...
  ok('Adopt platform "Microsoft 365 Copilot"', adopt.platform === 'Microsoft 365 Copilot');
  ...
  ok('Extend platform "Power Apps / Copilot Studio"', extend.platform === 'Power Apps / Copilot Studio');
  ...
  ok('Build platform "Azure AI Foundry Agent Service"', build.platform === 'Azure AI Foundry Agent Service');
  ...
  // L7: pill is the AWAF ladder tier, relabeled to avoid clashing with the
  // 6-gate flow stepper (this page = Gate 4). Expect "AWAF Tier: <tier>".
  ok('AWAF tier sub-label rendered', /AWAF Tier:\s*(Adopt|Low-code|Build)/.test(document.getElementById('vGateLabel').textContent));
  ...
  ok('phase platforms map correctly',
     JSON.stringify(phasePlats) === JSON.stringify(['Microsoft 365 Copilot','Power Apps / Copilot Studio','Azure AI Foundry Agent Service']));
  ...
  ok('advisors are Microsoft-flavored (Workspace Advocate / Platform Architect / Governance Lead)',
  ...
  ok('debateScript returns >= 3 turns for a tier', api.debateScript({tier:'Extend', verdictName:'Scale Smart', platform:'Power Apps / Copilot Studio'}).length >= 3);
  ...
  ok('all 5 AWAF dimensions present', dimKeys.every(k => typeof dims[k] === 'number'));
  ok('all dims within 1-5', dimKeys.every(k => dims[k] >= 1 && dims[k] <= 5));

  console.log('\n== 11. Data flow: reads keys + writes aaic_advisory ==');
  ok('page references aaic_feasibility key', /aaic_feasibility/.test(html));
  ok('page references aaic_bxt key', /aaic_bxt/.test(html));
  ok('page references aaic_intake key', /aaic_intake/.test(html));
  ok('page references aaic_advisory key', /aaic_advisory/.test(html));
  ok('loadIntake returns seeded intake (not demo)', api.loadIntake().fromDemo === false);
  const persisted = JSON.parse(window.localStorage.getItem('aaic_advisory'));
  ok('aaic_advisory persisted on load', persisted && typeof persisted.tier === 'string');
  ...
  window.localStorage.removeItem('aaic_advisory');
  click(window, document.getElementById('btnRerun'));
  ok('Re-run re-writes aaic_advisory', !!window.localStorage.getItem('aaic_advisory'));
  ok('Re-run keeps a valid tier', ['Adopt','Extend','Build'].indexOf(JSON.parse(window.localStorage.getItem('aaic_advisory')).tier) > -1);
  ...
  setTimeout(() => {
    ...
    ok('demo platform "Power Apps / Copilot Studio"', apiD.result.platform === 'Power Apps / Copilot Studio');
    ok('demo writes aaic_advisory', !!domD.window.localStorage.getItem('aaic_advisory'));

    console.log('\n== 15. Zero foreign-vendor strings; Microsoft framing present ==');
    ok('contains Avangrid framing (Copilot / Power Apps / Copilot Studio / Azure AI Foundry / Workspace)',
       /(Microsoft 365 Copilot|Power Apps \/ Copilot Studio|Azure AI Foundry Agent Service)/.test(html));
    const microsoft = ['Google','Gemini','Vertex','AppSheet','Agentspace','BigQuery','GADF','watsonx','Agentforce','Salesforce'];
    microsoft.forEach(m => ok('NO foreign-vendor string "'+m+'"',
